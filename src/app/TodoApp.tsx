...
export default function TodoApp({ user, onLogout, onUserUpdate }: { user: AppUser; onLogout: () => void; onUserUpdate: (updated: AppUser) => void }): React.ReactElement {
  ...
  async function recordWork(id: string): Promise<void> {
    ...
    console.log('[recordWork]', { id, addMin, memo });
    ...
    const workedAt: number = Date.now();
              ...
              {(workLogs[t.id] ?? []).map((wl: WorkLog) => (
                <li key={wl.id} className={styles.workLogItem}>
                  ...
                  <button
                    ...
                    onClick={() => {
                      // ログ本文から +N分 部分を抽出して、実績時間を巻き戻す
                      const deletedMin: number = extractMinutesFromLog(wl.content);
                      const targetTodo: Todo | undefined = todos.find((td: Todo) => td.id === t.id);
                      const prevActual: number = targetTodo?.actualMin ?? 0;
                      const newActual: number = Math.max(0, prevActual - deletedMin);

                      // 楽観的削除（ログ削除 + actualMin減算）
                      setWorkLogs((prev) => ({
                        ...prev,
                      ...
                      if (deletedMin > 0) {
                        setTodos((prev) =>
                          prev.map((td: Todo) => (td.id === t.id ? { ...td, actualMin: newActual } : td)),
                        );
                        setTodayMinMap((prev) => ({
                          ...prev,
                          [t.id]: Math.max(0, (prev[t.id] ?? 0) - deletedMin),
                        }));
                        fetch('/api/todos/' + t.id, {
                          method: 'PUT',
                          headers: { 'Content-Type': 'application/json' },
                          body: JSON.stringify({ updates: { actualMin: newActual } }),
                        });
                      }

                      // Undoトースト
                      const undoMessage: string = deletedMin > 0
                        ? `作業記録を削除しました（-${deletedMin}分を巻き戻し）`
                        : '作業記録を削除しました';
                      showUndoToast({
                        toastId: 'wl-' + wl.id,
                        todoId: t.id,
                        ...
                        undo: () => {
                          ...
                          if (deletedMin > 0) {
                            setTodos((prev) =>
                              prev.map((td: Todo) => (td.id === t.id ? { ...td, actualMin: prevActual } : td)),
                            );
                            setTodayMinMap((prev) => ({ ...prev, [t.id]: (prev[t.id] ?? 0) + deletedMin }));
                            fetch('/api/todos/' + t.id, {
                              method: 'PUT',
                              headers: { 'Content-Type': 'application/json' },
                              body: JSON.stringify({ updates: { actualMin: prevActual } }),
                            });
                          }
                          // DB復元（POSTで再作成）
          ...
          return (
              ...
              onTouchStart={(e) => {
                ...
                const nativeTouchMove = (ev: TouchEvent): void => {
                  ...
                  if (longPressTimerRef.current) {
                    ...
                    if (Math.abs(dx) > config.drag.moveThresholdPx || Math.abs(dy) > config.drag.moveThresholdPx) {
                      clearTimeout(longPressTimerRef.current);
                  ...
                  if (Math.abs(dx) > config.drag.swipeThresholdPx) {
                    ev.preventDefault();
                    setSwipeOffset((prev) => ({ ...prev, [t.id]: dx }));
                    if (dx < -config.drag.swipeNestThresholdPx) {
                      setSwipeAction((prev) => ({ ...prev, [t.id]: 'nest' }));
                    } else if (dx > config.drag.swipeNestThresholdPx && t.parentId) {
            ...
            >
              ...
              <div className={styles.actions}>
                ...
                <button onClick={() => recordWork(t.id)} className={styles.iconBtn} title="実績を加算" disabled={recordingId === t.id}>
                  +
                </button>
